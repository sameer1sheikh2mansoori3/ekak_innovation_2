"use client";

import Image from "next/image";
import React from "react";
import { CardBody, CardContainer, CardItem } from "@/components/ui/3d-card";
import Link from "next/link";

interface ThreeDCardProps {
  urlSection: string;
  title?: string;
  description?: string;
  linkText?: string;
  linkUrl?: string;
}

export function ThreeDCardDemo({
  urlSection,
  title = "Make things float in air", // Default title
  description = "Hover over this card to unleash the power of CSS perspective", // Default description
  linkText = "Try now →", // Default link text
  linkUrl = "https://twitter.com/mannupaaji", // Default link URL
}: ThreeDCardProps) {
  return (
    <CardContainer className="inter-var">
      <CardBody className="bg-black  relative group/card dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] dark:bg-black dark:border-white/[0.2] border-black/[0.1] w-auto sm:w-[30rem] h-auto rounded-xl p-6 border">
        <CardItem
          translateZ="50"
          className="text-xl font-bold   text-white"
        >
          {title}
        </CardItem>
        <CardItem
          as="p"
          translateZ="50"
          className="text-sm  mt-2 text-neutral-300"
        >
          {description}
        </CardItem>
        <CardItem translateZ="100" className="w-full mt-4">
          <Image
            src={urlSection}
            height="1000"
            width="1000"
            className="h-60 w-full object-cover rounded-xl group-hover/card:shadow-xl"
            alt="thumbnail"
          />
        </CardItem>
        <div className="flex justify-between items-center mt-20">
          <CardItem
            translateZ={20}
            as={Link}
            href={linkUrl}
            target="__blank"
            className="px-4 py-2 rounded-xl text-xs font-normal text-white"
          >
            {linkText}
          </CardItem>
        </div>
      </CardBody>
    </CardContainer>
  );
}
